...
struct Token {
  ...
  Token(TokenKind k, std::string const& text, Token* prev,
        SourceFile const* src, size_t pos)
      : kind(k), text(text), prev(prev), source(src), pos(pos) {
    if (prev) prev->next = this;
  }

  static Token from_str(std::string const& text,
  ...
  bool is_literal() const {
    return TokenKind::Int <= kind && kind <= TokenKind::String;
  }
