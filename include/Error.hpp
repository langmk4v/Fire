...
struct invalid_character_literal : e {
  invalid_character_literal(Token const& t)
      : e(t, "invalid character literal") {}
};
...
struct use_of_undefined_symbol : e {
  ...
  use_of_undefined_symbol(Token const& t, std::string const& scope,
...
struct ambiguous_symbol_name : e {
  ambiguous_symbol_name(Token const& t)
      : e(t, "'" + t.text + "' is ambigous") {}
};

struct empty_class_or_enum_is_not_valid : e {
  empty_class_or_enum_is_not_valid(Token const& t)
      : e(t, "empty class or enum is not valid") {}
};

struct no_match_template_arguments : e {
  no_match_template_arguments(Token const& t, int Expected, int Found)
      : e(t, format("expected %d template parameters, but found %d.", Expected,
                    Found)) {}
};
...
struct mismatched_types : e {
  ...
  mismatched_types(Token const& tok, std::string const& expected,
                   std::string const& found)
      : e(tok, "expected '" + expected + "' type expression, but found '" +
                   found + "'") {}
};

struct expected_item_of_module : e {
  expected_item_of_module(Token const& t)
      : e(t, "expected definition of function or class or enum after this "
             "token.") {}
};

struct mismatched_return_statement : e {
  mismatched_return_statement(Token const& t)
      : e(t, "mismatched return statement") {}
};

struct use_of_invalid_operator : e {
  use_of_invalid_operator(Token const& op, std::string const& left,
                          std::string const& right)
      : e(op, "use of invalid operator '" + op.text + "' for types '" + left +
                  "' and '" + right + "'") {}
};
...
struct index_out_of_range : e {
  index_out_of_range(Token const& t, int index, int size)
      : e(t, "index out of range") {
    (void)index;
    (void)size;
  }
};
...
struct import_not_allowed_here : e {
  import_not_allowed_here(Token const& t)
      : e(t, "import is not allowed here") {}
};

struct cannot_use_decltype_here : e {
  cannot_use_decltype_here(Token const& t)
      : e(t, "cannot use decltype() here") {}
};

struct import_depth_limit_exceeded : e {
  import_depth_limit_exceeded(Token const& t, std::string const& path)
      : e(t,
          "import depth limit exceeded by importing the path '" + path + "'") {}
};
...
struct cannot_declare_variable_here : e {
  cannot_declare_variable_here(Token const& t)
      : e(t, "cannot declare variable here") {}
};
} // namespace parses

namespace semantics {
struct this_is_not_typename : e {
  this_is_not_typename(Token const& t)
      : e(t, "'" + t.text + "' is not type name") {}
};

struct not_same_type_assignment : e {
  not_same_type_assignment(Token const& t, std::string const& dest,
...
struct cannot_use_typename_here : e {
  cannot_use_typename_here(Token const& t)
      : e(t, "cannot use type name here") {}
};
...
struct cannot_use_continue_here : e {
  cannot_use_continue_here(Token const& t)
      : e(t, "cannot use 'continue' here") {}
};
...
struct not_field_of_class : e {
  not_field_of_class(Token const& t, std::string const& Memb,
                     std::string const& Clas)
      : e(t, "'" + Memb + "' is not field of class '" + Clas + "'") {}
};
} // namespace semantics

namespace emitters {
static inline void expected_one_variant_for_enumerator(Token& tok,
                                                       NdEnumeratorDef* def) {
  err::e(tok,
         "expected one variant for enumerator '" + def->get_full_name() + "'",
         ET_Error)
      .print();
  err::e(def->variant->token, "defined here", ET_Note).print();
}

static inline void too_few_variants_for_enumerator(Token& tok,
                                                   NdEnumeratorDef* def) {
  err::e(tok, "too few variants for enumerator '" + def->get_full_name() + "'",
         ET_Error)
      .print();
  err::e(def->token, "defined here", ET_Note).print();
}

static inline void too_many_variants_for_enumerator(Token& tok,
                                                    NdEnumeratorDef* def) {
  err::e(tok, "too many variants for enumerator '" + def->get_full_name() + "'",
         ET_Error)
      .print();
  err::e(def->token, "defined here", ET_Note).print();
}

static inline void enumerator_no_have_variants(Token& tok,
                                               NdEnumeratorDef* def) {
  err::e(tok, "enumerator '" + def->get_full_name() + "' has no variants",
         ET_Error)
      .print();
  err::e(def->token, "defined here", ET_Note).print();
}
...
static inline void
tuple_getter_index_out_of_range(Token& index_tok, Token& tuple_obj_tok,
                                std::string const& tuple_type_str, int index,
                                int size) {
  err::index_out_of_range(index_tok, index, size).print();
  err::e(tuple_obj_tok,
         "tuple type: " COL_MAGENTA + tuple_type_str + COL_WHITE +
             " (maximum index is " + std::to_string(size - 1) + ")",
         ET_Note)
      .print(false);
}

static inline void expected_iterable_type(Token& tok, std::string const& type) {
  err::e(tok, "expected iterable type, but found '" + type + "'", ET_Error)
      .print();
}
} // namespace emitters
