...
static uint32_t utf8_decode_len(const unsigned char** p,
  ...
  if ((s[0] & 0xE0) == 0xC0 && s + 1 < end && (s[1] & 0xC0) == 0x80) {
    uint32_t cp = ((s[0] & 0x1F) << 6) | (s[1] & 0x3F);
    *p += 2;
    return (cp >= 0x80) ? cp : U_REPLACEMENT;
  }

  if ((s[0] & 0xF0) == 0xE0 && s + 2 < end && (s[1] & 0xC0) == 0x80 &&
      (s[2] & 0xC0) == 0x80) {
  ...
  if ((s[0] & 0xF8) == 0xF0 && s + 3 < end && (s[1] & 0xC0) == 0x80 &&
...
char16_t* utf8_to_utf16_with_len(char16_t* out, char const* input, size_t len) {
  ...
  if (!out) {
    out = (char16_t*)malloc((out_len + 1) * sizeof(char16_t));
    if (!out) return NULL;
  }
