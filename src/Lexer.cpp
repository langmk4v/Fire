...
static _token_punct_str_map_ const* find_punct(char const* ptr) {
  for (size_t i = 0; i < _psmap_table_size_; i++) {
    if (std::strncmp(ptr, _psmap_table_pointer_[i].str,
                     std::strlen(_psmap_table_pointer_[i].str)) == 0) {
...
Token* Lexer::tokenize(char c, Token* prev) {
  ...
  // char or string
    ...
    pass_space();
    ss.push_back(c);
    return new Token(kind, ss, prev, _source, pos);
  }

  else if (_token_punct_str_map_ const* p = find_punct(getptr());
           p != nullptr) {
    _pos += std::strlen(p->str);
    pass_space();
    auto tok = new Token(TokenKind::Punctuator, std::string(p->str), prev,
                         _source, pos);
    tok->punct = p->punct;
    return tok;
  }
  ...
  throw err::invalid_token(
      Token(TokenKind::Unknown, std::string(1, c), prev, _source, pos));
}
